...
class RegisterUnit(brica1.Module):
    class Register(brica1.Component):
        ...
        def fire(self):
            ...
            if len(in_data) < 2:
                in_data = np.zeros(self.dim, dtype=np.int)
            if self.token + 1 == self.inputs['token_in'][0]:
                self.recognized = np.array([0, 0])
                if True:    # np.array_equal(in_data, self.prev_in):
            ...
            if self.token < 2:  # TODO initialization error somewhere
                self.results['reward'] = np.array([0.0])
            else:
                self.results['reward'] = self.inputs['reward']
            self.results['done'] = self.inputs['done']
            if self.inputs['done'][0] == 1:
                self.results['token_out'] = np.array([0], dtype='int')
            else:
    ...
    class RegisterController(CBT1cCA.CBT1Component):
        def __init__(self, learning_mode, train, config):
            super().__init__(learning_mode, train, config)

    class RCStub(brica1.Component):
    ...
    def __init__(self, config, train):
        ...
        if config["stub"]:
            ...
            self.register_controller = RegisterUnit.RegisterController(config['learning_mode'], train, config)
...
class ActionDeterminer(CBT1cCA.CBT1Component):
    def __init__(self, learning_mode, train, config):
        super().__init__(learning_mode, train, config)


class ADStub(brica1.Component):
    def __init__(self, config):
        super().__init__()
        self.in_dim = config['in_dim']
        self.n_action = config['n_action']    # number of action choices
    ...
    def fire(self):
        ...
        if self.token + 1 == self.inputs['token_in'][0] or self.inputs['done'] == 1:
            in_data = self.get_in_port('observation').buffer
...
class MinWMAgent(brica1.Module):
    def __init__(self, config):
        ...
        if config['ActionDeterminer']['stub']:
            ...
            self.action_determiner = ActionDeterminer(config['ActionDeterminer']['learning_mode'],
